...
<div class="form-group {$error}">
	<label class="control-label col-md-2" for="{$defaults['name']}">{$label}</label>
	<div class="col-md-10">
		 <input {$output} {$extra} />
		{$tooltip}
	</div>
...
if ( ! function_exists('form_dropdown'))
{
	...
	function form_dropdown($data, $options=array(), $selected=array(), $label='', $extra='', $tooltip = '')
	{
		if (! is_array($data))
		{
			$data = array('name' => $data,'class' => 'form-control');
		}
...
<div class="form-group {$error}">
	<label class="col-md-2 control-label" for="{$data['id']}">{$label}</label>
		<div class="col-md-10">
		 <select {$output} {$extra}>
			{$options_vals}
		</select>
