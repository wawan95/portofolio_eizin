...
if ($db_required != '')
{
	$mb_index .= "
		...
		if(\$search){
			\$records  = \$this->".$module_name_lower."_model->search(\$search, \$limit, \$offset);
			\$total    = \$this->".$module_name_lower."_model->num_search(\$search);
		}
		else {
			\$records  = \$this->".$module_name_lower."_model->limit(\$limit, \$offset)->find_all();
			\$total    = \$this->".$module_name_lower."_model->count_all();
		}
...
$mb_index .= "
		Template::set('toolbar_title', '".$module_name."');
		Template::render();
	}
...
$mb_save =<<<END
	...
	public function search() {
		...
		Template::set('page', \$data);
		Template::set('records', \$records);
		Template::render('{$module_name_lower}/$controller_name_lower/search');
	}
