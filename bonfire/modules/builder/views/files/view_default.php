	...
	switch($field_type)
	{
		...
		case('textarea'):
			...
			<div class=\"form-group <?php echo form_error('{$field_name}') ? 'error' : ''; ?>\">
				<?php echo form_label('{$field_label}'{$required}, '{$form_name}', array('class' => 'col-md-2 control-label col-md-2') ); ?>
				<div class='col-md-10'>
					<?php echo form_textarea( array( 'name' => '{$form_name}', 'id' => '{$form_name}', 'class'=>'form-control', 'rows' => '5', 'cols' => '80', 'value' => set_value('$form_name', isset(\${$module_name_lower}['{$field_name}']) ? \${$module_name_lower}['{$field_name}'] : '') ) ); ?>
		...
		case('radio'):
			...
			$view .= PHP_EOL . "
			<div class=\"form-group <?php echo form_error('{$field_name}') ? 'error' : ''; ?>\">
				<?php echo form_label('{$field_label}'{$required}, '', array('class' => 'col-md-2 control-label col-md-2', 'id' => '{$form_name}_label') ); ?>
				<div class='col-md-10' aria-labelled-by='{$form_name}_label'>
		...
		case('checkbox'):
			...
			$view .= PHP_EOL . "
			<div class=\"form-group <?php echo form_error('{$field_name}') ? 'error' : ''; ?>\">
				<?php echo form_label('{$field_label}'{$required}, '{$form_name}', array('class' => 'control-label col-md-2 col-md-2') ); ?>
				<div class='col-md-10'>
		...
		default: // input.. added bit of error detection setting select as default
			...
			<div class=\"form-group <?php echo form_error('{$field_name}') ? 'error' : ''; ?>\">
				<?php echo form_label('{$field_label}'{$required}, '{$form_name}', array('class' => 'control-label col-md-2 col-md-2') ); ?>
				<div class='col-md-10'>
					<input id='{$form_name}' type='{$type}' name='{$form_name}' class='col-md-10 form-control' {$maxlength} value=\"<?php echo set_value('{$form_name}', isset(\${$module_name_lower}['{$field_name}']) ? \${$module_name_lower}['{$field_name}'] : ''); ?>\" />
					<span class='help-inline'><?php echo form_error('{$field_name}'); ?></span>
				</div>
			</div>";
