...
class Contexts
{
	 ...
	 * @var string
	 */
	protected static $outer_class	= 'sidebar-menu'; //class <ul> pertama 
	 ...
	 * @var string
	 */
	protected static $outer_id	= ''; // id <ul> pertama
	 ...
	 * @var string
	 */
	protected static $parent_class	= 'treeview'; //class <li> yang punya sub
	 ...
	 * @var string
	 */
	protected static $child_class	= 'treeview-menu'; //class <ul> menu 2\
	 ...
	 * @var string
	 */
	protected static $submenu_class	= ''; //class <li> menu 2

	/**
	 * The class to apply to ul2 tags within li2 tags.
	 *
	 * @access protected
	 * @static
	 *
	 * @var string
	 */
	protected static $child3_class	= 'treeview-menu';
	...
	/**
	 * Stores the context icon config.
	 *
	 * @access protected
	 * @static
	 *
	 * @var array
	 */
	protected static $context_icons = array();
	...
	protected static function init()
	{
		...
		self::$context_icons = self::$ci->config->item('context_icons');
	...
	public static function render_menu($mode='text', $order_by='normal', $top_level_only = FALSE)
	{
		...
		$context_icons = self::$context_icons;
		...
		$nav .= '<li class="treeview"><a href="'.base_url("admin").'"><i class="fa fa-dashboard fa-fw"></i> <span>Beranda</span></a></li>';
		...
		*/
		foreach ($contexts as $context)
		{
			if ( has_permission('Site.'. ucfirst($context) .'.View') == true || permission_exists('Site.'. ucfirst($context) .'.View') == false)
			{
				...
				$nav .= "<li class='" . self::$parent_class . " {$class}'>
							<a href='{$url}' id='{$id}' class='dropdown-toggle' title='{$title}' data-toggle='dropdown' data-id='{$context}_menu'>";
				...
				{
					if (array_key_exists(strtolower($context), $context_icons)){
						$nav .= '<i class="fa '.$context_icons[$context].' fa-fw"></i> ';
					}
					$nav .= "<span>$title</span>";
				}

				$nav .= "<i class='fa fa-angle-left pull-right'></i></a>";
	 ...
	 * @return 	bool
	 */
	public static function create_context($name='', $icon='', $roles=array(), $migrate=false)
	{
		if (empty($name))
		{
		...
		*/
		...
		if (!in_array(strtolower($name), $contexts))
		{
			...
			if (!empty($icon))
			{
				$stack = array(strtolower($name) => $icon);
				$context_icons = array_merge($context_icons, $stack);
				write_config('application', array('context_icons' => $context_icons), null);
			}
	...
	//--------------------------------------------------------------------
	public static function delete_context($name='', $roles=array()) 
	{
		if (empty($name))
		{
			self::$errors = lang('ui_no_context_name');
		...
		if (in_array(strtolower($name), $contexts))
		{
			...
			if (array_key_exists(strtolower($name), $context_icons)){
				unset($context_icons[$name]);
				write_config('application', array('context_icons' => $context_icons), null);
			}	
			//array_unshift($contexts, strtolower($name));
			
			if (!write_config('application', array('contexts' => $new_contexts), null))
			{
				self::$errors[] = lang('ui_cant_write_config');
				return false;
			}
		...
		*/
		...
		if (self::$ci->permission_model->permission_exists($cname))
		{
			$perm = self::$ci->permission_model->find_by('name', $cname);
		
			$pid = $perm->permission_id;
		
			self::$ci->permission_model->delete_by_name($cname);
		}
		...
		foreach ($roles as $role)
		{
			...
			{
				self::$ci->role_permission_model->unssign_to_role($role, $cname);
			}
		}
	...
	public static function build_sub_menu($context, $ignore_ul=FALSE)
	{
		...
		foreach (self::$menu as $topic_name => $topic)
		{
			...
			if (count($topic) > 1)
			{
				//$list .= '<li class="' . self::$submenu_class . '"><a href="#" ><i class="fa fa-circle-o"></i>'. ucwords($topic_name) .' <i class="fa fa-angle-left pull-right"></a>';
				$list .= '<li><a href="#" ><i class="fa fa-circle-o"></i>'. ucwords($topic_name) .' <i class="fa fa-angle-left pull-right"></i></a>';
				$list .= '<ul class="' . self::$child_class .'">';
	...
	private static function build_item($module, $title, $display_name, $context, $menu_view='')
	{
		if (empty($menu_view))
		{
			...
			$item .= ' title="'. $title .'"><i class="fa fa-caret-right"></i> '. ucwords(str_replace('_', '', $display_name)) ."</a>\n";
		...
		else
		{
			...
			$item = '<li><a href="#" ><i class="fa fa-plus-square"></i>'. ucwords(str_replace('_', '', $display_name)) .'<i class="fa fa-angle-left pull-right"></i></a>';
			$item .= '<ul class="' . self::$child3_class .'">';
