...
<div id="login">
	...
	<?php echo form_open(LOGIN_URL, array('autocomplete' => 'off')); ?>
		...
		<div class="form-group <?php echo iif( form_error('login') , 'error') ;?>">
			<div class="col-md-10">
				<input style="width: 95%" type="text" name="login" id="login_value" value="<?php echo set_value('login'); ?>" tabindex="1" placeholder="<?php echo $this->settings_lib->item('auth.login_type') == 'both' ? lang('bf_username') .'/'. lang('bf_email') : ucwords($this->settings_lib->item('auth.login_type')) ?>" autofocus />
			</div>
		</div>
