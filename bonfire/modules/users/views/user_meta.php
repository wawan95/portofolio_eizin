		<?php foreach ($meta_fields as $field):?>
			...
			elseif ($field['form_detail']['type'] == 'checkbox'): ?>
					...
					<label class="control-label col-md-2 col-md-2" for="<?php echo $field['name'] ?>"><?php echo $field['label'];?></label>
					<div class="col-md-10">
						<?php
			...
			<?php elseif ($field['form_detail']['type'] == 'state_select' && is_callable('state_select')) : ?>
				...
				<div class="form-group <?php echo iif( form_error($field['name']) , 'error'); ?>">
						<label class="control-label col-md-2 col-md-2" for="<?php echo $field['name'] ?>"><?php echo lang('user_meta_state'); ?></label>
						<div class="col-md-10">

							<?php echo state_select(set_value($field['name'], isset($user->$field['name']) ? $user->$field['name'] : 'SC'), 'SC', 'US', $field['name'], 'form-control chzn-select'); ?>
				...
				<?php elseif ($field['form_detail']['type'] == 'country_select' && is_callable('country_select')) : ?>
					...
					<div class="form-group <?php echo iif( form_error('country') , 'error'); ?>">
						<label class="control-label col-md-2 col-md-2" for="country"><?php echo lang('user_meta_country'); ?></label>
						<div class="col-md-10">
							<?php echo country_select(set_value($field['name'], isset($user->$field['name']) ? $user->$field['name'] : 'US'), 'US', 'country', 'form-control chzn-select'); ?>
